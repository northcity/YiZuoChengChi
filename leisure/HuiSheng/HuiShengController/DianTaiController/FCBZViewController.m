...
#import "FCBZDViewController.h"

#define FCBZURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=67&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"

@interface FCBZViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation FCBZViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:FCBZURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    FCBZDViewController *lmc = [[FCBZDViewController alloc]initWithModel:model];
