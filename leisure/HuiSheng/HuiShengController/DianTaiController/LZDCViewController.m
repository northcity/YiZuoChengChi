...
#import "LZDCDViewController.h"
#define LZDCURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=118&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface LZDCViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation LZDCViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:LZDCURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    LZDCDViewController *lmc = [[LZDCDViewController alloc]initWithModel:model];
