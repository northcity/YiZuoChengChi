...
#import "QGDViewController.h"

#define QGURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=176&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface QGViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:QGURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    QGDViewController *lmc = [[QGDViewController alloc]initWithModel:model];
