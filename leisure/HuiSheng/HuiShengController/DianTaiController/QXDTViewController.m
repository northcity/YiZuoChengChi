...
#import "QXDTDViewController.h"
#define QXDTURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=482&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"

@interface QXDTViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation QXDTViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:QXDTURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    QXDTDViewController *lmc = [[QXDTDViewController alloc]initWithModel:model];
