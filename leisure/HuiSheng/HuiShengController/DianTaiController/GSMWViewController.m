...
#import "GSMWDViewController.h"
#define GSMWURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=117&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"

@interface GSMWViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation GSMWViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:GSMWURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    GSMWDViewController *lmc = [[GSMWDViewController alloc]initWithModel:model];
