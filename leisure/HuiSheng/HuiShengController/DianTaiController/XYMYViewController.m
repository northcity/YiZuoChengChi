...
#define XYMY @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=68&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"


@interface XYMYViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:XYMY,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    XYMYDViewController *lmc = [[XYMYDViewController alloc]initWithModel:model];
