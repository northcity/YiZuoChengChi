...
#define QGYHURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=115&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface QGYHViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation QGYHViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:QGYHURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    QGYHDViewController *lmc = [[QGYHDViewController alloc]initWithModel:model];
