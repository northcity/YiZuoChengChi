...
#define XYURLLLL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=184&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface XYViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:XYURLLLL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    XYDViewController *lmc = [[XYDViewController alloc]initWithModel:model];
