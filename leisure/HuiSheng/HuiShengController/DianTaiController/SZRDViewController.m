...
#import "SZRDDViewController.h"

#define SZRDURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=100&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface SZRDViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation SZRDViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:SZRDURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    SZRDDViewController *lmc = [[SZRDDViewController alloc]initWithModel:model];
