...
#import "YXJMDViewController.h"

#define YXJMURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=478&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface YXJMViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation YXJMViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:YXJMURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    YXJMDViewController *lmc = [[YXJMDViewController alloc]initWithModel:model];
