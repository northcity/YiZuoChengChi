...
#import "GYSJDViewController.h"

#define GYSJURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=434&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface GYSJViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation GYSJViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:GYSJURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    GYSJDViewController *lmc = [[GYSJDViewController alloc]initWithModel:model];
