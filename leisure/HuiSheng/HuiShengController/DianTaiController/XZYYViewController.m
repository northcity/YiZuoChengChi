...
#import "XZYYDViewController.h"


#define XZYYURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=70&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface XZYYViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation XZYYViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:XZYYURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    XZYYDViewController *lmc = [[XZYYDViewController alloc]initWithModel:model];
