...
#import "LXYYDViewController.h"

#define LXYYURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=484&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"

@interface LXYYViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation LXYYViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:LXYYURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    LXYYDViewController *lmc = [[LXYYDViewController alloc]initWithModel:model];
