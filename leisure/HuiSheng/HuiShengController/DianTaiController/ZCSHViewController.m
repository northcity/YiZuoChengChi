...
#import "ZCSHDViewController.h"
#define ZCSHURL @"http://www.duole.fm/api/category/get_collect_list?app_version=23&category_id=114&device=android&finish=0&limit=20&page=%ld&rm_empty=1&sort=3&visitor_uid=256492"
@interface ZCSHViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation ZCSHViewController
...
- (void)getNetData
{
    ...
    NSString *url = [NSString stringWithFormat:ZCSHURL,self.page];
    //NSLog(@"%@",url);
...
{
    LXSMModel *model = self.dataSource[indexPath.row];
    ZCSHDViewController *lmc = [[ZCSHDViewController alloc]initWithModel:model];
